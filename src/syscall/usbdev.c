 ...
 *     kIOReturnNoDevice/NotAttached on any op marks every usbfs fd open on
 *     that device disconnected, not only the one that noticed -- the walk
 *     usbdev_remove does over udev->filelist: poll -> POLLERR|POLLHUP,
 *     REAPURB hands back what the post-disconnect kill recovers and then
 *     -ENODEV, every other ioctl -ENODEV. A non-blocking REAPURBNDELAY
 *     issues that kill's aborts and returns without waiting for them.
 ...
 * that page (it is the same devio.c:1182 bound), the URB stands in the way the
...
static void usbdev_mark_peers_disconnected_tlocked(usbdev_t *u)
{
    ...
    for (int i = 0; i < USBDEV_MAX_FDS; i++) {
        ...
        if (o == u || !o->used || o->dead || devkey_load(o) != key)
            continue;
                                    ...
                                    IOUSBInterfaceInterface800 **intf)
{
    ...
    for (;;) {
        ...
        if (pthread_cond_timedwait(&u->async_cv, &u->async_lock, &deadline) ==
             ...
             * kill/teardown scan cannot re-find it and wait another 2s. The
             * process-wide budget is not settled here: nurbs and inflight_bytes
             * are slot fields a reused slot must not see moved, and
             * usbdev_memory_usage is neither -- it accounts live bytes, and
             * r->buf is still allocated and still owned by an in-flight IOKit
             * transfer. It is refunded where that buffer is actually released,
             * in urb_free_orphan_locked.
             */
            int n = 0;
            r = u->pending_head;
            while (r) {
            ...
            /* Restart every FIFO, exactly as the success path below does.
             * Clearing draining is not enough: an endpoint whose leader
             * completed during the drain took the orphaned early return in
             * usbdev_async_cb, which starts nothing, so it is left with a
             * queued URB and none in flight and no later event to restart it --
             * its REAPURB never returns.
             */
...
static void usbdev_teardown_locked(usbdev_t *u)
{
    ...
    /* The pipe goes with the fd, so the token goes with it; the two disconnect
     * flags reset together here for the same reason they do in the open path.
     */
    u->ready_token = false;
...
static void usbdev_fd_cleanup(int guest_fd)
{
    ...
    /* This path unlocks by hand rather than through usbdev_release, so it owes
     * the same flush: a teardown whose kill found the device gone has peers to
     * tell, and the closing fd is the one that will never ask again.
     */
    usbdev_flush_disc_peers(u);
...
int64_t usbdev_open_path(const char *path, int linux_flags)
{
    ...
    /* Both halves of the disconnect state, together: a slot whose previous open
     * drained after a disconnect would otherwise start this open with the drain
     * already claimed, and this open's own disconnect would answer -ENODEV with
     * its URBs still pending. Nothing else clears either flag -- usbdev_init
     * zeroes the table once, at startup.
     */
    u->disconnected = false;
    u->disc_drained = false;
    u->discsig_signr = 0;
 ...
 * the URB the way it does on Linux (devio.c:2025).
 */
 ...
 * follows reap_as (devio.c:2081-2119): wake on completion or disconnect, -EINTR
...
static int64_t usbdev_do_reap(guest_t *g, int fd, uint64_t arg, bool block)
{
    for (;;) {
         ...
         *
         * "Finds the completion list empty" is the whole of the condition, not
         * a description of the common case: a pass that has a completion to
         * hand back returns from the if (rec) arm below, so a drain claimed on
         * such a pass never runs its kill, and every later pass then sees the
         * flag already set and answers -ENODEV with the in-flight URBs still
         * unheard from -- the opposite of what the capability promises.
         */
        bool drain =
            !rec && disc && !u->disc_drained && u->pending_head != NULL;
        if (drain)
            u->disc_drained = true;
        ...
        host_fd_ref_close(&ready);
        if (rc < 0) {
            /* reap_as returns -EINTR with no restart (devio.c:2116-2117). */
            syscall_restart_forbid();
            return rc;
        }
    }
