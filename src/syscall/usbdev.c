 ...
 * (research doc D's op table). Semantics mirror drivers/usb/core/devio.c
 * (doc A §A1/§A2):
 *
 ...
 *   - CLAIMINTERFACE returns -EBUSY when a kernel driver is bound; the
 ...
 *   - CONTROL/BULK are the sync kernel paths (do_proc_control/do_proc_bulk):
 *     bounce buffers around DeviceRequestTO / Read|WritePipeTO, timeout in ms
 *     (0 = unlimited on both sides), -ETIMEDOUT from
 *     kIOUSBTransactionTimeout, stall -> -EPIPE, and the kernel's implicit
 ...
 *     IOUSBLib.h "BadArgument if TO on interrupt pipe"). TODO(stage 3): route
 ...
 *     TODO(stage 3): explicit dup alias (fuse_dup_fd pattern).
 ...
 * transfer-shape caps a Linux 7.x kernel always sets (libusb keys URB
 * splitting and ZLP behaviour off them) and deliberately clears MMAP,
 * DROP_PRIVILEGES, CONNINFO_EX, and SUSPEND, which name ioctls this layer
 * does not serve (doc A §A7.7).
 */
...
/* Linux poll bits (asm-generic/poll.h). POLLWRNORM differs from macOS
 * (0x100 vs 0x004), so the guest-facing remap must use these, never the
 * host's <poll.h> values.
 */
...
/* One SUBMITURB. Guest data bounces through buf: copy-in at submit and
 * copy-out at reap both happen on the vCPU thread; the completion callback
 * (event thread) touches only this record and its owning static fd slot.
 */
...
typedef struct usbdev {
    ...
    /* Guards every field above except used/guest_fd/generation, which the
     * table lock guards. Held across blocking IOKit transfer calls, which
     * serializes concurrent ioctls on one fd exactly like the kernel's
     * per-device lock (dropped-during-transfer refinement is a TODO).
     */
    pthread_mutex_t lock;

    /* --- async URB state, guarded by async_lock (never held across a
     * blocking IOKit call other than the callback-side ZLP WritePipe). The
     * completion callback and the disconnect notification run on the event
     * thread and take ONLY this lock, so a vCPU thread parked in a sync
     * transfer under `lock` cannot stall completions on other endpoints.
     * Lock order: lock -> async_lock. pipe_wr may additionally be written
     * under async_lock (teardown sets it to -1 under both locks before
     * closing).
     */
 ...
 * remap helpers: they run on hot poll paths and must not queue behind an
 * entry lock held across a blocking sync transfer. Set by the event thread
 * at disconnect, cleared when the guest fd is bound or torn down. A stale
 * bit on a reused fd number is harmless: the helpers check the fd type
 * first, and binding a new usbdev fd clears it.
 */
...
/* Retained IOUSBHostInterface service for bInterfaceNumber ifnum in the
 * active configuration, or IO_OBJECT_NULL. Uses CreateInterfaceIterator so
 * "exists" means exactly what claimintf's usb_ifnum_to_if means.
 */
 ...
 * (CreateInterfaceAsyncEventSource) sources are added to it from vCPU
 * threads, exactly libusb's darwin model (darwin_usb.c:1894-1911,
 * 2261-2272). Chosen over the CreateDeviceAsyncPort + mach_msg alternative
 * because the runloop also carries the IONotificationPort for disconnect
 * interest messages with no extra plumbing.
 *
 * The thread is started lazily on the first async submit and never joined:
 * exec keeps the host process (side tables survive), and after
 * guest_destroy the callbacks only touch usbdev-owned host memory (static
 * fd slots, malloc'd URB records, the completion pipe), never guest
 * memory -- the copy-in/copy-out happens on vCPU threads (netlink.c:853-857
 * warning).
 */
...
/* Event-thread-safe disconnect stamp: async_lock only, no guest memory.
 * Wakes pollers with one extra pipe byte (the remap turns it into
 * POLLERR|POLLHUP) and REAPURB waiters via the cv.
 */
static void usbdev_mark_disconnected(usbdev_t *u)
{
    ...
    if (!u->disconnected) {
        u->disconnected = true;
        /* guest_fd is table-lock-guarded; this unlocked read races only
         * with teardown, which clears the map bit again afterwards.
         */
        int gfd = u->guest_fd;
                               ...
                               void *arg)
{
    ...
    if (msg == kIOMessageServiceIsTerminated)
        usbdev_mark_disconnected((usbdev_t *) refcon);
}

/* Register the terminate-interest notification (entry lock held). Best
 * effort: kIOReturnNoDevice detection on ops is the fallback.
 */
static void usbdev_arm_disconnect_watch(usbdev_t *u)
{
    ...
    if (!port)
        return;
    if (IOServiceAddInterestNotification(port, u->service, kIOGeneralInterest,
                                         usbdev_interest_cb, u,
                                         &u->notif) != kIOReturnSuccess)
...
/* kIOReturn -> linux URB status (doc D table (b); differs from the sync
 * ioctl map in the Aborted row: DISCARDURB = usb_kill_urb = -ENOENT, any
 * other abort = async unlink = -ECONNRESET).
 */
...
/* Hand rec to IOKit. Called with async_lock held: the async entry points do
 * not block, and their callbacks arrive later on the event thread.
 */
static IOReturn usbdev_urb_start(usbdev_urb_t *rec)
{
     ...
     * pipe_interrupt set, i.e. the kernel's silent bulk->interrupt
     * conversion (devio.c:1718-1721).
     */
 ...
 * nothing is in flight there; locally fail records IOKit refuses
 * (async_lock held).
 */
...
/* IOKit completion (event thread). arg0 carries the transferred byte count
 * for pipe reads/writes and wLenDone for device requests.
 */
static void usbdev_async_cb(void *refcon, IOReturn result, void *arg0)
{
    ...
    if ((uint32_t) result == (uint32_t) kIOReturnNoDevice ||
        ...
        usbdev_mark_disconnected(u); /* takes async_lock; call first */
    pthread_mutex_lock(&u->async_lock);
    ...
    /* ZLP: a maxpacket-multiple OUT gets its terminating zero-length packet
     * as a separate synchronous WritePipe from the callback
     * (darwin_usb.c:3193-3204). Blocks the event thread for one packet
     * time, same as libusb.
     */
...
/* Abort and drain every URB whose interface matches intf (NULL = all,
 * including ep0). QUEUED records complete as killed (-ENOENT, usb_kill_urb)
 * and stay reapable; INFLIGHT ones are aborted and drained through the
 * callback. Returns false when the drain timed out: survivors were orphaned
 * (the late callback frees them) and the caller must NOT release the IOKit
 * handles they still reference. Entry lock held.
 */
...
/* Free every reapable completion (fd close: the kernel frees unreaped
 * completed URBs too). Entry lock held.
 */
...
static int64_t usbdev_release_locked(usbdev_t *u, unsigned ifnum)
{
    ...
    usbdev_iface_t *fi = &u->ifaces[ifnum];
    if (!fi->claimed)
        return -LINUX_EINVAL; /* releaseintf: not claimed */
    ...
    if (drained) {
         ...
         * source; leak both rather than hand the late callback a freed
         * plugin.
         */
 ...
 * bEndpointAddress ep, searching every altsetting. Parsed from the
 * descriptors blob. Returns -1 when not found.
 */
...
static usbdev_t *usbdev_acquire(int fd)
{
    ...
    /* Take the entry lock before dropping the table lock (table -> entry
     * order) so the slot cannot be torn down and reallocated in between. The
     * cost: while one thread's sync transfer holds the entry lock, a second
     * ioctl on the SAME fd queues here with the table lock held, briefly
     * stalling other fds' lookups. The kernel serializes per-device too; the
     * cross-fd stall is a stage-2 simplification.
     */
    if (u)
...
{
    /* Async teardown first: kill/drain every URB (release close: kernel
     * kills pending and frees completed, devio.c:1092-1128), then the
     * notification and the ep0 event source, so no callback can arrive for
     * this slot after the handles go away.
     */
    ...
    /* Orphaned callbacks skip the pipe write, so closing it here is safe
     * even on the timeout path; -1 under async_lock keeps the callback's
     * check and this close ordered.
     */
    pthread_mutex_lock(&u->async_lock);
    int pw = u->pipe_wr;
    u->pipe_wr = -1;
...
{
    /* Find and tear down under table -> entry so a concurrent teardown +
     * slot reuse cannot hand this call somebody else's live entry. A sync
     * transfer in flight on this fd holds the entry lock, so this close
     * waits for it (with the table lock held; see usbdev_acquire's note).
     */
    pthread_mutex_lock(&usbdev_table_lock);
    usbdev_t *u = NULL;
    for (int i = 0; i < USBDEV_MAX_FDS; i++) {
...
int64_t usbdev_open_path(const char *path, int linux_flags)
{
    ...
    /* fd_alloc publishes the slot before the side table binds the guest fd;
     * a guest racing close() on a guessed fd number in that window leaks this
...
static int64_t usbdev_do_control(usbdev_t *u, guest_t *g, uint64_t arg)
{
    ...
    if ((ct.bRequestType & 0x60) != 0x40) {
        ...
        if (recip == 1) { /* interface */
            ...
            uint8_t pipe = 0;
            int64_t rc =
                usbdev_pipe_for_ep(u, (uint8_t) (ct.wIndex & 0x8f), &fi, &pipe);
            if (rc < 0)
                return rc;
            (void) fi;
            (void) pipe;
...
static int64_t usbdev_do_bulk(usbdev_t *u, guest_t *g, uint64_t arg)
{
    ...
    if (guest_read_small(g, arg, &bt, sizeof(bt)) < 0)
        return -LINUX_EFAULT;
    if ((bt.ep & 0x70) != 0 || bt.len > USBDEV_BULK_MAX)
        return -LINUX_EINVAL;
    ...
    if (type == kUSBInterrupt) {
         ...
         * TODO(stage 3): async submit + timed wait.
         */
        log_warn(
            "usbdev: sync BULK on interrupt ep 0x%02x unsupported "
            "(stage 3)",
            bt.ep);
        return -LINUX_EINVAL;
    }
...
static int64_t usbdev_do_getdriver(usbdev_t *u, guest_t *g, uint64_t arg)
{
    ...
    if (u->ifaces[gd.interface].claimed) {
        ...
        if (ifs == IO_OBJECT_NULL)
            return -LINUX_EINVAL; /* usb_ifnum_to_if NULL */
...
static int64_t usbdev_do_setconfiguration(usbdev_t *u, guest_t *g, uint64_t arg)
{
    ...
    if (cfg > 255)
        return -LINUX_EINVAL;
    /* proc_setconfig: -EBUSY when ANY interface is claimed, usbfs included
     * (devio.c:1561-1578).
     */
    for (int i = 0; i < USBDEV_MAX_IFACES; i++)
        if (u->ifaces[i].claimed)
...
static int64_t usbdev_do_clear_halt(usbdev_t *u, guest_t *g, uint64_t arg)
{
    ...
    if (rc < 0)
        return rc;
    /* ClearPipeStallBothEnds == CLEAR_FEATURE(ENDPOINT_HALT) + host-side
     * toggle reset (IOUSBLib.h:2928-2941), exactly usb_clear_halt.
     */
...
static int64_t usbdev_do_resetep(usbdev_t *u, guest_t *g, uint64_t arg)
{
     ...
     * ClearPipeStallBothEnds is the closest IOKit equivalent (it also sends
     * the wire CLEAR_FEATURE, a benign superset).
     */
...
static int64_t usbdev_do_disconnect_claim(usbdev_t *u, guest_t *g, uint64_t arg)
{
    ...
    if (bound) {
        ...
        if (strcmp(drv, "usbfs") != 0) {
             ...
             * privileges-dropped kernel answer (devio.c:2482).
             */
            return -LINUX_EACCES;
        }
    }
...
static int64_t usbdev_do_submiturb(usbdev_t *u, guest_t *g, uint64_t arg)
{
    ...
    if (uu.type == LINUX_URB_TYPE_CONTROL) {
        ...
        if ((uint32_t) uu.buffer_length - 8 < wLength)
            return -LINUX_EINVAL;
        /* check_ctrlrecip: vendor requests bypass; IF/EP recipients claim
         * the owning interface implicitly (devio.c:881-938).
         */
        if ((setup[0] & 0x60) != 0x40) {
            ...
            if (recip == 1) {
                ...
                usbdev_iface_t *efi = NULL;
                uint8_t epipe = 0;
                int64_t rc = usbdev_pipe_for_ep(u, (uint8_t) (wIndex & 0x8f),
                                                &efi, &epipe);
                if (rc < 0)
                    return rc;
            }
    ...
    rec->is_in = is_in;
    /* Direction-mismatched flags are ignored, not rejected (devio.c
     * honours SHORT_NOT_OK for IN and ZERO_PACKET for OUT only,
     * 1710-1737).
     */
    ...
    if (!busy) {
        /* Endpoint idle: hand it to IOKit now. Queued follow-ups start from
         * the completion callback (one in-flight URB per endpoint keeps
         * DISCARDURB per-URB, see file header).
         */
...
/* DISCARDURB (proc_unlinkurb): arg is the user URB pointer. Entry lock
 * held. A QUEUED record completes locally as killed (-ENOENT); an INFLIGHT
 * one is flagged and aborted -- with one in-flight URB per endpoint the
 * AbortPipe cannot cancel bystanders. Kernel usb_kill_urb is synchronous;
 * here the -ENOENT completion arrives via the callback, which REAPURB then
 * observes (deviation noted in the file header).
 */
static int64_t usbdev_do_discardurb(usbdev_t *u, uint64_t arg)
{
    ...
    pthread_mutex_unlock(&u->async_lock);
    /* The interface handle cannot be released concurrently: release paths
     * need the entry lock this thread holds.
     */
    if (pipe == 0)
        (void) (*u->dev)->USBDeviceAbortPipeZero(u->dev);
    else
...
/* Copy one completion back to the guest (vCPU thread): IN data into the
 * urb's buffer, then status/actual_length/error_count into the guest urb,
 * then the userurb pointer into *arg (processcompl, devio.c:2042-2078).
 */
...
/* REAPURB / REAPURBNDELAY. Called WITHOUT the entry lock held so a blocked
 * reap never stalls submits or discards; each pass revalidates the fd.
 * Blocking follows reap_as (devio.c:2080-2118): wake on completion or
 * disconnect, -EINTR without restart on a signal, -ENODEV once
 * disconnected and drained (REAP_AFTER_DISCONNECT).
 */
static int64_t usbdev_do_reap(guest_t *g, int fd, uint64_t arg, bool block)
{
    for (;;) {
        ...
        if (rec) {
             ...
             * entry lock still pins the pipe's host fd open (cleanup waits
             * on it).
             */
            char b;
                             ...
                             short *host_events)
{
    ...
    if (!fd_snapshot(guest_fd, &snap) || snap.type != FD_USBDEV)
        return false;
    /* POLLOUT needs FMODE_WRITE (devio.c:2833); a disconnected fd must
     * still wake the host poll (the disconnect byte) to surface
     * POLLERR|POLLHUP.
     */
    bool writable = (snap.linux_flags & LINUX_O_ACCMODE) != LINUX_O_RDONLY;
    short ev = 0;
    if ((writable &&
         (guest_events & (LINUX_POLLOUT | LINUX_POLLWRNORM)) != 0) ||
        usbdev_fd_disconnected(guest_fd))
        ev = POLLIN;
    *host_events = ev;
    return true;
}
...
int64_t usbdev_ioctl(guest_t *g, int fd, uint64_t request, uint64_t arg)
{
    ...
    /* The reaps manage their own locking: a blocked REAPURB must not hold
     * the entry lock against concurrent SUBMITURB/DISCARDURB, and they stay
     * usable after disconnect (devio.c:2612-2635).
     */
    ...
    case USBDEVFS_DISCSIGNAL: {
        /* Stored for fidelity but never delivered (file header): elfuse has
         * no async guest-signal injection from the event thread.
         */
