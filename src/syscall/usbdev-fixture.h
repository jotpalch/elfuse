 ...
 *
 * Every entry point below answers for the fixture only. In a run that did not
 * ask for it, usbdev_fixture_loopback() is false, usbdev_fixture_open_device
 * refuses a location the fixture does not model, and nothing else is reachable.
 *
 * Two translation units define them. usbdev-fixture.c is the model, and is
 * compiled only when USB_LOOPBACK_FIXTURE asks for it (mk/config.mk);
 * usbdev-fixture-stub.c is the answer every other build links, and models
 * nothing. usbdev.c calls these names either way and has no conditional
 * compilation of its own, which is what keeps the two builds one program.
 */
 ...
 * -LINUX_ENOENT for an interface number the fixture device does not carry, and
 * -LINUX_ENODEV once the modeled device has terminated, because IOKit fails all
 * three of those steps against a service that is gone.
 */
