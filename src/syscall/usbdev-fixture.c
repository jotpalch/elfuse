 ...
 *   never        accepted and never completed until something aborts the pipe,
 *                which is what DISCARDURB and every kill do. That is faithful
 *                to AbortPipe, and it is also why never cannot reach the drain
 *                deadline: the abort it answers promptly is the drain.
 *   wedge(ms)    never, plus an abort that takes ms to land (default 2500,
 *                past the engine's 2 s drain deadline). This is the transfer
 *                the deadline and the orphaning exist for, and the only step
 *                that reaches them.
 ...
 *   0x40 0xF2  terminate the device in wValue milliseconds. A wIndex naming
 *              either bit below arms no terminate at all: it leaves the device
 *              in place and breaks one call apiece, which is how a departure
 *              only one IOKit entry point can see is modeled. Bit 1 tears down
 *              the claimed interface's pipes, so GetPipeProperties alone
 *              answers NoDevice: the device vanishing between the
 *              GetNumEndpoints that sizes a pipe map and the calls that fill it
 *              in. Bit 2 makes AbortPipe and USBDeviceAbortPipeZero answer
 *              NoDevice, which is the abort a teardown drain cannot get issued
 *              at all. Both bits are cleared by 0xF3.
 *   0x40 0xF3  clear the wire log and its clock, the loopback data stash, the
 *              pending zlpfail steps, the 0xF4 counter and both 0xF2 wIndex
 *              bits above
 *   0xC0 0xF4  read the counters the fixture keeps about the layer above it
 *              rather than about the wire, 4 bytes little-endian. One so far:
 *              how many times the device interface was Released with a control
 *              transfer still outstanding on it.
 */
...
/* Past usbdev.c's 2 s drain deadline with room for a loaded machine. */
#define FX_WEDGE_ABORT_MS 2500
...
typedef struct {
    ...
    bool zlp_fail[256];
    bool pipes_gone; /* GetPipeProperties alone answers NoDevice */

    /* AbortPipe and USBDeviceAbortPipeZero answer kIOReturnNoDevice and cancel
     * nothing. Its own fact, and not d->terminated, for the reason the seam
     * invariant at fx_create_iface_iterator gives: an abort after the device
     * leaves is precisely what the disconnect drain issues and IOKit lands it,
     * so tying these to the terminate would leave every 'never' transfer
     * outstanding for ever and no drain would finish. What this models is the
     * narrower answer -- the user client is gone, so the abort aborts nothing
     * -- which is the one state usbdev_do_discardurb had no way to reach and
     * the one it used to discard.
     */
    bool aborts_gone;

    /* Device handles released while a transfer this fixture still owns is
     * outstanding on the default control pipe. IOKit would answer a later
     * callback against a freed user client; the fixture frees a COM wrapper
     * nothing dereferences again, so the use-after-free itself is unreachable
     * here and this counter is what stands in for it. Read back through
     * FX_CMD_STATS, so a scenario can assert on it after the fd is gone.
     */
    uint32_t dev_release_inflight;
    uint64_t t0_ms;
    struct {
        bool used;
                         ...
                         const void *payload)
{
    ...
    if (payload && (r->flags & 1u)) {
        /* actual is whatever the script said, and a script may report more than
         * the transfer asked for -- ok(16) against a four-byte IN URB. payload
         * is the submitted buffer, so the peek is bounded by what was requested
         * as well as by the field: reading to actual walked off the allocation.
         */
        uint32_t n = actual < r->requested ? actual : r->requested;
        if (n > 8)
            n = 8;
                          ...
                          void *refcon)
{
    ...
    if (st.act != FX_NEVER && st.act != FX_WEDGE) {
...
static IOReturn fx_abort(fx_dev_t *d, uint8_t ep)
{
    ...
    for (int i = 0; i < FX_MAX_XFERS; i++) {
        ...
        /* AbortPipe returns before the cancellation lands, and how long that
         * takes is the host controller's business: wedge is the transfer whose
         * abort outlives the engine's 2 s drain deadline, which is the only way
         * in-tree to reach the orphaning path.
         */
        uint32_t late = 0;
        if (x->act == FX_WEDGE)
            late = x->has_arg ? x->arg : FX_WEDGE_ABORT_MS;
        if (x->timer && late == 0)
            CFRunLoopTimerSetNextFireDate(x->timer, CFAbsoluteTimeGetCurrent());
        else if (!x->timer)
            x->timer = fx_arm(late, fx_timer_cb, x);
    }
    if (fx_loop)
        CFRunLoopWakeUp(fx_loop);
    pthread_mutex_unlock(&usbdev_fixture_lock);
...
#define FX_CMD_STATS 0xf4

/* wIndex flags on FX_CMD_TERMINATE (see the control plane above) */
#define FX_TERM_PIPES 0x2u
#define FX_TERM_ABORTS 0x4u
...
static bool fx_command(fx_dev_t *d, IOUSBDevRequestTO *req, IOReturn *out)
{
    ...
    *out = kIOReturnSuccess;
    ...
    case FX_CMD_SCRIPT: {
        ...
        /* Refused rather than truncated. Loading the prefix installed a rule
         * set the scenario did not write -- the rules past the cut simply gone
         * -- while reporting the whole request as transferred, so a scenario
         * that outgrew the buffer ran as a different scenario and looked like a
         * pass. A script that does not fit is a mistake in the test, and this
         * is what makes it say so.
         */
        if (req->wLength > sizeof(buf) - 1) {
            log_warn("usbdev fixture: script of %u bytes exceeds %zu",
                     (unsigned) req->wLength, sizeof(buf) - 1);
            *out = kIOReturnBadArgument;
            return true;
        }
    ...
    case FX_CMD_TERMINATE: {
        pthread_mutex_lock(&usbdev_fixture_lock);
        if (req->wIndex & (FX_TERM_PIPES | FX_TERM_ABORTS)) {
            if (req->wIndex & FX_TERM_PIPES)
                d->pipes_gone = true;
            if (req->wIndex & FX_TERM_ABORTS)
                d->aborts_gone = true;
            pthread_mutex_unlock(&usbdev_fixture_lock);
            return true;
        }
    ...
    case FX_CMD_STATS: {
        /* Counters the fixture keeps about the layer above it, rather than
         * about the wire. One so far: fx_release_dev's.
         */
        pthread_mutex_lock(&usbdev_fixture_lock);
        uint32_t v = d->dev_release_inflight;
        pthread_mutex_unlock(&usbdev_fixture_lock);
        if (req->pData && req->wLength >= 4) {
            fx_put32(req->pData, v);
            req->wLenDone = 4;
        }
        return true;
    }
...
static ULONG fx_release_dev(void *self)
{
    ...
    pthread_mutex_lock(&usbdev_fixture_lock);
    for (int i = 0; i < FX_MAX_XFERS; i++) {
        if (fx_xfers[i].used && fx_xfers[i].dev == o->dev &&
            fx_xfers[i].ep == 0) {
            o->dev->dev_release_inflight++;
            break;
        }
    }
    pthread_mutex_unlock(&usbdev_fixture_lock);
...
/* SetConfiguration is a SET_CONFIGURATION request on the wire, so it answers a
 * terminated device the way the transfers do: the seam invariant is written out
 * at fx_create_iface_iterator below.
 */
static IOReturn fx_set_configuration(void *self, UInt8 cfg)
{
    fx_dev_t *d = fx_of_dev(self);
    pthread_mutex_lock(&usbdev_fixture_lock);
    bool gone = d->terminated;
    pthread_mutex_unlock(&usbdev_fixture_lock);
    if (gone)
        return kIOReturnNoDevice;
...
/* Invariant for the whole seam: an entry that stands for a request on the wire
 * or for a question put to the device's user client answers kIOReturnNoDevice
 * once the device has terminated, because that is what IOKit answers. An entry
 * that stands for anything else says so here, by name, with its reason -- a
 * model that keeps answering where the device would not is what lets a
 * regression in the layer above land green.
 *
 * The entries deliberately outside it, and why:
 *
 *   GetNumEndpoints, GetPipeProperties  served from descriptor state the
 *     interface user client already holds, not from the wire, so termination
 *     is not what stops them; what stops GetPipeProperties is the pipes being
 *     torn down, which is its own fact and its own flag.
 *   AbortPipe, USBDeviceAbortPipeZero  an abort after the device leaves is
 *     precisely what the disconnect drain issues, and IOKit lands it. Tying
 *     these would leave every 'never' transfer outstanding forever.
 *   USBInterfaceClose, USBDeviceClose  the release half of a handle whose
 *     device has gone, which teardown has to be able to do. IOKit does not
 *     refuse a close against a terminated service, and usbdev.c ignores the
 *     result at all three call sites in any case, so a refusal would model
 *     nothing.
 *   Create{Device,Interface}AsyncEventSource  local plumbing over the mach
 *     port rather than a call on the device, and completions are delivered
 *     through the source it returns.
 *   USBInterfaceOpen  one of the three steps usbdev_fixture_open_iface stands
 *     for, and tied there.
 *   USBDeviceOpen  the one entry the rule covers and the code does not, and
 *     not because IOKit answers differently. usbdev_lazy_device_open
 *     tolerates the refusal but still translates it, so a NoDevice here would
 *     stamp the fd from inside the same call that arms the terminate watch, and
 *     the late-delivery arm in usbdev_fixture_watch -- the whole reason a fresh
 *     fd on a departed device gets a wake at all -- would have nothing left to
 *     be observed by. Which of the two should originate is a question about
 *     that arm, not about this seam, and it is not settled here.
 */
                                         ...
                                         io_iterator_t *iter)
{
    ...
    *iter = IO_OBJECT_NULL;
    ...
    bool gone = d->terminated;
    pthread_mutex_unlock(&usbdev_fixture_lock);
    return gone ? kIOReturnNoDevice : kIOReturnSuccess;
}
...
/* ClearPipeStallBothEnds sends CLEAR_FEATURE(ENDPOINT_HALT) on the wire, so it
 * is inside the invariant above.
 */
...
/* SetAlternateInterface is a SET_INTERFACE request on the wire. */
...
/* Inside the invariant at fx_create_iface_iterator: this stands for the
 * interface service lookup, the plugin creation and USBInterfaceOpen, and IOKit
 * fails all three against a service that has terminated.
 *
 * The answer is an errno rather than an IOReturn, so it carries no disconnect
 * stamp of its own, and it does not need one: usbdev_claim_locked puts the
 * device question ahead of the branch that reaches here, so a claim on a
 * departed device is already -ENODEV with the fd stamped before this is called.
 * What this is for is the model telling the truth, so that dropping that
 * question makes the loopback lane fail instead of pass.
 */
                          ...
                          void *refcon)
{
    ...
    /* IOServiceAddInterestNotification against a service that has already
     * terminated still delivers the terminate message. Recording the watcher
     * and saying nothing left the fd with no wake coming at all, so a poll or
     * epoll wait on it could block for its whole timeout.
     *
     * Re-run the same delivery rather than calling cb from here: this runs
     * under usbdev.c's per-entry lock, and usbdev_interest_cb takes
     * usbdev_table_lock, which internal.h says is never held together with the
     * entry lock. The event thread is where IOKit would have called it and
     * where fx_deliver_terminate already calls it, holding neither. Watchers
     * that were notified the first time are notified again, which costs
     * nothing: the mark is idempotent.
     */
    if (d->terminated && free_slot >= 0)
        (void) fx_arm(0, fx_terminate_cb, d);
    pthread_mutex_unlock(&usbdev_fixture_lock);
}
