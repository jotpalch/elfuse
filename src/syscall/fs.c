 ...
 * /proc/self/fd/N (or /dev/fd/N) magic link reopens the underlying file, so the
 * new descriptor must inherit the target fd's stamped identity, not the literal
 * magic-link spelling: fstatfs on a reopened /sys directory has to keep
 * answering SYSFS_MAGIC (systemd's sd-device reopens every chased syspath this
 * way and gates on it), and a reopened regular file must not masquerade as
 * procfs.
 *
 * Returns path unchanged for non-magic-link paths, the target's stamp copied
 * into buf, or NULL when the target has no virtual identity (stamp nothing).
 */
