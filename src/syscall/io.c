 ...
 * for pending output before TCSBRK, TCSBRKP and TIOCSBRK, and a pending signal
 * aborts the wait with a plain -EINTR that reaches the guest without a restart
 * (tty_io.c "Factor out some common prep work" block). macOS tcdrain() would
 * park the vCPU thread beyond the reach of guest signals -- a serial port
...
static int64_t tty_drain_interruptible(int host_fd)
{
    ...
    for (;;) {
        ...
        if (interrupted) {
            /* The kernel returns a plain -EINTR here (not ERESTARTSYS), and
             * part of the interval has already drained: no restart.
             */
...
int64_t sys_ioctl(guest_t *g, int fd, uint64_t request, uint64_t arg)
{
    ...
    case LINUX_TCSBRK: {
        /* Linux overloads TCSBRK: the kernel always waits for pending output
    ...
    case LINUX_TCSBRKP: {
         ...
         * means 250 ms). The kernel drains pending output before both TCSBRK
         * and TCSBRKP; macOS tcsendbreak() does not, and ignores the duration
         * (fixed ~0.4 s), which is close enough for a break.
         */
        int64_t rc = tty_drain_interruptible(host_fd);
    ...
    case LINUX_TIOCCBRK: {
         ...
         * semantics on macOS; a pty answers ENOTTY on both systems. The kernel
         * drains pending output before TIOCSBRK (but not TIOCCBRK), same as
         * TCSBRK.
         */
        int64_t rc = 0;
    ...
    case LINUX_TIOCMBIS:
    case LINUX_TIOCMBIC: {
        /* The kernel masks the set/clear words to the output lines
         ...
         * the kernel with a TIOCMBIS/TIOCMBIC pair. The first host call is
         * issued even when its masked word is empty so a fd whose driver lacks
         * modem control still reports ENOTTY, as Linux does before looking at
         * the value.
         */
        int32_t val = 0;
