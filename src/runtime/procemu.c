...
int proc_intercept_readlink(const char *path, char *buf, size_t bufsiz)
{
    ...
    if (!strncmp(path, "/proc/self/fd/", 14)) {
         ...
         * scratch dirs under /tmp; F_GETPATH would leak that host location,
         * and consumers compare the magic-link target against the guest path
         * they opened (systemd chase() rejects a syspath that does not start
         * with /sys). Report the stamped guest spelling instead. Kept narrow
         * (USB prefixes only) so the pty and shm reporting stays as it was.
         */
        {
            fd_entry_t fd_snap;
     ...
     * symlinks; answer EINVAL for existing paths rather than falling through
     * to the host (where /sys does not exist and the error would be ENOENT).
     */
    {
