 ...
 * boot-time enumeration order for a fixed device set. Neither guarantee depends
 * on the order the caller lists the keys in.
 *
 * Keys longer than TTY_ALIAS_KEY_MAX-1 bytes are truncated for matching, so two
 * names sharing that prefix share one slot; duplicates in keys collapse to one
 * slot. When the pool is full only NEW keys go unassigned (tty_alias_pool_index
 * answers -1 for them) -- a key already holding a slot keeps it however many
 * keys the caller lists.
 */
