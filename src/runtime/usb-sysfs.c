 ...
 * stat intercept reports them as char major 189 minor (bus-1)*128+(dev-1),
 * and the open intercept diverts them (the /dev/pts placeholder trick).
 *
...
/* Port path from locationID nibbles below the top (bus) byte; empty for a
 * root hub (nusb parse_location_id). Returns the number of ports written.
 */
...
/* Enumerate the IOKit registry into usb_devs[]. Devices whose locationID has
 * no port nibbles are root hubs; they are skipped (no usbN entries: nusb
 * drops them and libusb tolerates a missing parent).
 */
static void model_build(void)
{
    ...
    while ((svc = IOIteratorNext(it))) {
        ...
        /* macOS bus indices (locationID top byte) start at 0; Linux busnums
         * are 1-based (minor arithmetic and BBB names break at bus 0), so
         * shift by one. Deviation from a literal locationID>>24 is deliberate
         * and stable: the top byte is constant per controller.
         */
        ...
        /* Fall back to the first config's bConfigurationValue when the
         * registry has no current-configuration key.
         */
    ...
    /* Fallback devnum assignment: stable 1..n per bus in locationID order
     * for devices without a 'USB Address' property, avoiding taken numbers.
     */
 ...
 * setting 0) of the active configuration, parsed from the raw config
 * descriptor already in the blob -- registry children vanish under capture,
 * so they are deliberately not consulted.
 */
                                     ...
                                     const char **sys_suffix_out)
{
    if (path_prefix_match(path, "/sys", 4)) {
        /* /sys/devices/system/cpu stays with the syscpu stub, which the
         * procemu dispatchers consult before this layer.
         */
...
/* Synthetic stat identities, mirroring procemu.c's PROC_SYNTH_DEV scheme with
 * a distinct device so /sys/bus/usb nodes never collide with /proc ones.
 */
...
{
    memset(st, 0, sizeof(*st));
    /* 0664 rather than udev's root:root 0660 policy: the single-user guest
     * must be able to open its own device nodes.
     */
...
/* Synthetic descriptor-blob fd (the proc_synthetic_fd pattern): unlinked
 * temp file so lseek/pread work.
 */
...
int usb_sysfs_intercept_readlink(const char *path, char *buf, size_t bufsiz)
{
    ...
    if (kind == USB_PATH_SYS) {
         ...
         * readlink passes their targets through. Plain dirs/files answer
         * EINVAL and missing paths ENOENT, exactly as sysfs would.
         */
        pthread_mutex_lock(&usb_lock);
        int rc = -1;
        int err = 0;
