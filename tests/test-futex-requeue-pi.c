...
/* Mirrors futex_bucket_index in proved/futexhash.h; test-futex-waitv-buckets.c
 * carries the same constants.
 */
#define FUTEX_HASH_MULT 0x9E3779B97F4A7C15ULL
#define FUTEX_BUCKETS 1024u

static uint32_t bucket_of(const void *addr)
{
    uint64_t a = (uint64_t) (uintptr_t) addr;
    return (uint32_t) (((((a >> 2) * FUTEX_HASH_MULT) >> 32)) % FUTEX_BUCKETS);
}

/* The lock under test, a second address distinct enough to land in a different
 * bucket, and the four handshake words that sequence the two threads around
 * them.
 */
static int pi_lock;       /* the PI futex the waiter parks on */
static int pi_lock_dst;   /* requeue destination for the cross-bucket case */
...
static long futex_requeue(int *addr, int *dst, long wake, long requeue)
{
    return raw_syscall6(__NR_futex, (long) addr,
...
{
    return futex_requeue(addr, addr, wake, requeue);
}
...
int main(void)
{
    ...
    /* On a collision, the cross-bucket case below silently repeats the
     * same-bucket one and still passes, so the split gets its own assertion
     * rather than being left implicit.
     */
    TEST("pi_lock and pi_lock_dst land in different buckets");
    if (bucket_of(&pi_lock) == bucket_of(&pi_lock_dst))
        FAIL("addresses collided; the cross-bucket case below would be moot");
    else
        PASS();
    ...
    /* pi_lock_dst is a distinct address, so this crosses two buckets rather
     * than the one lock the same-address cases above take. The rejection is on
     * the waiter alone, so the destination bucket must not matter.
     */
    TEST("PI waiter inside the wake budget, distinct destination");
    EXPECT_RAW_ERRNO(
        futex_requeue(&pi_lock, &pi_lock_dst, 1, 1), -EINVAL,
        "a PI waiter within wake_count must be EINVAL across buckets too");
