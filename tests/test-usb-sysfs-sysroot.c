 ...
 * asserted together, and through a dirfd and a cwd as well as absolutely: a
 * descriptor on /dev has to carry its guest spelling or openat(dirfd,
 * "ttyACM0") reaches the placeholder file the alias node sits on top of, and a
 * cwd on /dev is decided by a test of its own, which the alias directories were
 * missing from -- chdir("/dev") then stat("ttyACM0") reached that same
 * placeholder while both other spellings reached the character device.
 */
...
int main(void)
{
     ...
     *
     * Through a cwd as well as through a descriptor, because the two are
     * decided by different tests: a descriptor carries the guest spelling
     * stamped on it, while a relative lookup against the cwd is matched against
     * a test of its own. With the alias directories absent from that test,
     * chdir("/dev") then stat("ttyACM0") reported this sysroot's placeholder
     * file where /dev/ttyACM0 and the dirfd lookup reported the character
     * device. fchdir is asked beside chdir: chdir has the directory's name and
     * fchdir has only a descriptor, so one can be served where the other is
     * not.
     */
