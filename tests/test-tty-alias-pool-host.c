...
/* The duplicate collapse, with an assertion that can tell it apart from not
 * collapsing at all. The scan above cannot: two copies of one key land on the
 * same slot either way, so deleting the collapse left it passing. Fill the pool
 * with copies of one name and add one distinct name behind them -- collapsed,
 * the distinct name is the second key and takes slot 1; uncollapsed, the copies
 * consume the pool and it gets nothing.
 */
static void test_duplicates_do_not_consume_slots(void)
{
    tty_alias_pool_t pool;
    tty_alias_pool_init(&pool);
    const char *scan[TTY_ALIAS_POOL_SLOTS + 1];
    for (int i = 0; i < TTY_ALIAS_POOL_SLOTS; i++)
        scan[i] = "cu.usbmodemSAME";
    scan[TTY_ALIAS_POOL_SLOTS] = "cu.usbmodemZZZZ";
    tty_alias_pool_rescan(&pool, scan, TTY_ALIAS_POOL_SLOTS + 1);
    assert(tty_alias_pool_index(&pool, "cu.usbmodemSAME") == 0);
    assert(tty_alias_pool_index(&pool, "cu.usbmodemZZZZ") == 1);
}
...
/* A full pool rescanned with the newcomer listed FIRST. The order matters and
 * the case above cannot see it: it lists the 64 low names first and the extras
 * last, which is the benign order. Listed the other way round, the rescan used
 * to survey only the first TTY_ALIAS_POOL_SLOTS keys of the input -- so the
 * last attached key fell outside the survey, was treated as departed, had its
 * slot freed, and the newcomer took it. Both promises in the header broke at
 * once: an attached device lost its index and a name in use was reassigned.
 */
static void test_pool_full_keeps_attached_keys_whatever_the_order(void)
{
    tty_alias_pool_t pool;
    tty_alias_pool_init(&pool);
    char names[TTY_ALIAS_POOL_SLOTS][24];
    const char *scan[TTY_ALIAS_POOL_SLOTS + 1];
    for (int i = 0; i < TTY_ALIAS_POOL_SLOTS; i++) {
        snprintf(names[i], sizeof(names[i]), "cu.usbmodem%03d", i);
        scan[i] = names[i];
    }
    tty_alias_pool_rescan(&pool, scan, TTY_ALIAS_POOL_SLOTS);
    int last = tty_alias_pool_index(&pool, names[TTY_ALIAS_POOL_SLOTS - 1]);
    assert(last == TTY_ALIAS_POOL_SLOTS - 1);

    /* The newcomer first, every attached key behind it. */
    scan[0] = "cu.usbmodemNEW";
    for (int i = 0; i < TTY_ALIAS_POOL_SLOTS; i++)
        scan[i + 1] = names[i];
    tty_alias_pool_rescan(&pool, scan, TTY_ALIAS_POOL_SLOTS + 1);

    for (int i = 0; i < TTY_ALIAS_POOL_SLOTS; i++)
        assert(tty_alias_pool_index(&pool, names[i]) == i);
    assert(tty_alias_pool_index(&pool, "cu.usbmodemNEW") == -1);
}
...
int main(void)
{
    ...
    test_duplicates_do_not_consume_slots();
    test_long_keys_truncate_consistently();
    test_pool_full_leaves_extras_unassigned();
    test_pool_full_keeps_attached_keys_whatever_the_order();
