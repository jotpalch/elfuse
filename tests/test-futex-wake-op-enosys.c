 ...
 * within 5 yields under elfuse and 23 on the reference kernel, worst of 600
 * runs each, so this count is a margin and not a tuned number.
 */
 ...
 */
static void unsupported_op_case(const char *name, unsigned op, unsigned cmp)
{
    TEST(name);
    __atomic_store_n(&target_word, 0, __ATOMIC_RELEASE);
    long rc = futex_wake_op(1, 1, encode(op, cmp, 0x111, 0x111));
    if (rc != -ENOSYS) {
...
int main(void)
{
    ...
    unsupported_op_case("op 5 is ENOSYS", 5, 0);
    unsupported_op_case("op 6 is ENOSYS", 6, 0);
    unsupported_op_case("op 7 is ENOSYS", 7, 0);
    ...
    unsupported_op_case("an unsupported pair stops at the op", 5, 6);
